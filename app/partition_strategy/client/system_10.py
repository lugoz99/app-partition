...
from app.partition_strategy.data.supermatriz import get_matriz_10
...
def get_evaluate_system(
...
) -> Tuple[List[str], OrderedSet, List[int], Dict[str, List[int]], np.ndarray]:
    """
    ...
    # Extraer variables y estados
    parte_t = subsistema_seleccionado.split("|")[-1]
    var_names = list(re.findall(r"[A-Z]", parte_t))
    current_values = [
        int(estado_inicial[sistema_completo.index(var)]) for var in var_names
    ]

    # Construir `v` y `z`
    v = OrderedSet([f"{var.lower()}t" for var in var_names] + [f"{var.lower()}t+1" for var in var_names])
